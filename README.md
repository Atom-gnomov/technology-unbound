...
**Фаза 1 (0.1)** — каркас: модульная система, EnergyCanon, аспекты предметов
IC2, вкладка Таумономикона, рецепты дробителя/компрессора/экстрактора,
алюментум-топливо.

**Фаза 3а (0.2)** — конвертеры энергии: Таум-Генератор (узел → EU),
Эфирный Двигатель (EU → узел), Таум-Сталь, 4 исследования.

Обе фазы написаны, но **ещё ни разу не собраны** — первая локальная сборка
проверит обращения к IC2 API (см. `docs/IC2_API_ASSUMPTIONS.md`).
Дорожная карта: `docs/integration/ic2_v5_impl_skeleton.md` в репо порта.
